...
R packages always contain a `DESCRIPTION` file in the project root with package metadata, but also non-package projects can use a `DESCRIPTION` file to declare which packages they rely on.

The `Imports` fields lists packages that the package depends on and that will be installed (or updated) when the package is installed using `install.packages()`.
